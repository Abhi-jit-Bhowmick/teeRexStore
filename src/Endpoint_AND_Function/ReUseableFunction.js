...
*/
export const initialFilterCheckBox = {
    color: [],
    gender: [],
    price: [],
    type: [],
...
export function makeArrayByProperty(products, type) {
...
/*
const price = ["0 - Rs.250","Rs.251 - 450"]

export const sortByPrice = (price,products) => {
    let prod = price.map((item) => {
        if (item === "0 - Rs.250") {
            return products.filter((p) => {
                return p.price > 0 && p.price <= 250
            })
        }else if (item === "Rs.251 - 450") {
            return products.filter((p) => {
...
*/
// console.log(sortByPrice(allProducts))


export const ItemIsStock = ({quantity,qty}) => {
    if (quantity < 1 || quantity === qty) {
        ...
        return (
            <div className="in_stock">
                <p>
                    {quantity-qty === 1 ? (
                        `${quantity -qty} item is available`
                    ) : (
                        `${quantity -qty} items are available`
                    )}
                </p>
